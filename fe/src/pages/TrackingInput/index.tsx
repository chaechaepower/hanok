  ...
  return (
    ...
    >
      <div
        ...
        style={{
          backgroundColor: '#fff', borderRadius: '20px', padding: '40px 32px 32px',
          width: '360px', position: 'relative', boxShadow: '0 20px 60px rgba(0,0,0,0.25)',
      ...
      >
        <button
          ...
          style={{
            ...
            border: 'none', cursor: 'pointer', color: '#888', padding: '4px',
          }}
        >
          <FiX size={20} />
        </button>
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <textarea
            ...
            rows={4}
            style={{
              width: '100%', padding: '14px', fontSize: '14px', color: '#1A2238',
              border: '1px solid #E5E5EA', borderRadius: '10px', resize: 'none',
        ...
        <button
          ...
          style={{
            width: '100%', height: '52px', backgroundColor: '#1A2238', color: '#fff',
...
export default function TrackingInput() {
  ...
  const completedItems = items.filter((item) => item.escrowState === 'INVOICE_SUBMITTED' || item.escrowState === 'COMPLETED');
                ...
                {cancelledItems.map((item) => (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                      <div style={{ width: '80px', height: '80px', backgroundColor: '#2C2C2E', borderRadius: '12px', overflow: 'hidden', flexShrink: 0 }}>
                        {item.imageUrl
                          ? <img src={item.imageUrl} alt={item.itemName} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(100%)' }} />
                          : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#8E8E93', fontSize: '12px' }}>이미지 준비중</div>
                        }
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <div style={{ backgroundColor: '#1C1C1E', borderRadius: '24px', padding: '32px', border: '1px solid #3A3A3C', display: 'flex', flexDirection: 'column', minHeight: '600px' }}>
              <div style={{ display: 'flex', gap: '20px', marginBottom: '32px' }}>
                <div style={{ width: '120px', height: '120px', backgroundColor: '#2C2C2E', borderRadius: '16px', overflow: 'hidden', flexShrink: 0 }}>
                  {selectedItemDetail.winningInfo.imageUrl
                    ? <img src={selectedItemDetail.winningInfo.imageUrl} alt={selectedItemDetail.winningInfo.itemName} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#8E8E93', fontSize: '14px' }}>이미지 준비중</div>
                  }
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', flex: 1 }}>
                  <p style={{ fontSize: '20px', fontWeight: 'bold', marginBottom: '8px', wordBreak: 'keep-all', lineHeight: '1.3' }}>{selectedItemDetail.winningInfo.itemName}</p>
                  <p style={{ color: '#8E8E93', fontSize: '13px', marginBottom: '16px' }}>{selectedItemDetail.winningInfo.wonAt}</p>
                  <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr', gap: '8px 12px', fontSize: '14px' }}>
                    <span style={{ color: '#AEAEB2' }}>낙찰가</span>
                    <span style={{ textAlign: 'right', fontWeight: '500' }}>{formatPrice(selectedItemDetail.winningInfo.finalPrice)}</span>
                    <span style={{ color: '#AEAEB2' }}>구매자</span>
                    <span style={{ textAlign: 'right' }}>{selectedItemDetail.winningInfo.sellerName}({selectedItemDetail.winningInfo.sellerId})</span>
                  </div>
                </div>
              </div>
              ...
              {selectedItemDetail.delivery && (
                <div style={{ backgroundColor: '#151517', borderRadius: '12px', padding: '16px 24px', border: '1px solid #2C2C2E', marginBottom: '16px', fontSize: '14px', color: '#E5E5EA', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontWeight: '600' }}>{selectedItemDetail.delivery.courierName} | {selectedItemDetail.delivery.trackingNumber}</span>
                </div>
              )}
              ...
              {isTrackingSubmitted ? (
                <div style={{ backgroundColor: '#2C2C2E', borderRadius: '12px', padding: '20px', border: '1px solid #3A3A3C', textAlign: 'center', color: '#8E8E93', fontSize: '14px' }}>
                  운송장이 등록된 상품입니다.
                </div>
              ) : (
                <>
                  <div style={{ display: 'flex', gap: '8px', marginBottom: '32px' }}>
                    <select
                      ...
                      style={{
                        width: '140px', height: '48px', backgroundColor: 'transparent',
                        border: '1px solid #3A3A3C', borderRadius: '8px', color: 'white',
                        padding: '0 12px', fontSize: '14px', appearance: 'none', outline: 'none',
                    ...
                    >
                      ...
                      {COURIER_LIST.map((company) => (
                        <option key={company.Code} value={company.Code} style={{ backgroundColor: '#1C1C1E', color: 'white' }}>
                          {company.Name}
                        </option>
                      ))}
                    </select>
                    <input
                      ...
                      style={{
                        flex: 1, height: '48px', backgroundColor: 'transparent',
                        border: '1px solid #3A3A3C', borderRadius: '8px', color: 'white',
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '12px' }}>
                    <button
                      onClick={() => {
                        ...
                        submitTracking(
                          ...
                          {
                            onSuccess: () => { showToast({ message: '운송장 번호가 성공적으로 등록되었습니다.' }); },
                            onError: () => { showToast({ message: '운송장 번호 등록에 실패했습니다. 다시 시도해주세요.' }); }
                          }
                        );
                      }}
                      style={{
                        width: '120px', height: '48px', backgroundColor: 'white', color: 'black',
                    ...
                    <button
                      ...
                      style={{
                        backgroundColor: 'transparent', border: 'none', color: '#8E8E93',
