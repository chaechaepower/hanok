...
import { LuVolume2, LuVolumeOff, LuWallet } from "react-icons/lu";
...
export default function BuyerControlBar() {
    ...
    return (
        <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between">
            ...
            {/* 하단 중앙: 입찰 패널 */}
            <div
                className="flex flex-1 flex-col gap-2 mx-4 h-[180px] rounded-2xl bg-[rgba(0,0,0,.6)] px-4 py-3"
            ...
            >
                ...
                <div className="relative h-[2px] w-full shrink-0">
                    ...
                    <input
                        ...
                        className="absolute left-0 right-0 -top-2 h-5 w-full cursor-pointer opacity-0"
                    />
                </div>
                {/* 탭 */}
                        ...
                        <div className="flex flex-1 gap-2">
                            <div className="flex flex-1 flex-col justify-end">
                                <div className="px-1 text-right">
                                    <div className="text-[10px] text-[#71717a]">현재 최고가</div>
                                    <div className="text-base font-black tabular-nums text-white">
                                        {currentBid.toLocaleString()}
                                        <span className="text-xs font-normal text-[#a1a1aa]">원</span>
                                    </div>
                                </div>
                                <div className="flex items-center justify-between rounded-lg bg-[#18181b] px-3 py-1">
                        ...
                        <div className="flex flex-1 flex-col gap-2">
                            ...
                            <div className="flex gap-1.5">
                                {CUSTOM_UNIT_OPTIONS.map((opt) => (
                                    <button
                                        key={opt.label}
                                        className={`flex-1 rounded-lg py-1.5 text-[11px] font-bold transition ${customUnit === opt.value
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg transition ${isFreeMode ? "bg-[#18181b] text-[#52525b]" : "bg-[#27272a] text-white hover:bg-[#3f3f46]"
                                        }`}
                                ...
                                {isFreeMode ? (
                                    <input
                                        ...
                                        placeholder="금액 입력"
                                        className="flex-1 bg-transparent text-center text-lg font-black tabular-nums text-white outline-none placeholder:text-[#52525b]"
                                    />
                                ) : (
                                    <div className="flex-1 text-center text-lg font-black tabular-nums text-white">
                                        {bidAmount.toLocaleString()} <span className="text-sm font-normal text-[#a1a1aa]">원</span>
                                    </div>
                                )}
                                <button
                                    className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg transition ${isFreeMode ? "bg-[#18181b] text-[#52525b]" : "bg-[#27272a] text-white hover:bg-[#3f3f46]"
                            ...
                            {/* 잔고 */}
                            <div className="flex items-center justify-between rounded-lg bg-[#18181b] px-3 py-1.5">
