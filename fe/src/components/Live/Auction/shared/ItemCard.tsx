...
function ItemCard({ item, isSelected, onSelect, disabled }: ItemCardProps) {
    ...
    return (
        ...
        >
            {/* 진행중 glow */}
            {isActive && (
                <span className="absolute inset-0 rounded-xl ring-1 ring-amber-400/20 pointer-events-none" />
            )}
...
export default function Demo() {
    ...
    const doneItems = MOCK_ITEMS.filter(
        (i) => i.status === "sold" || i.status === "unsold"
    );
            ...
            <div className="w-full max-w-sm space-y-4">
                ...
                <div className="flex items-center justify-between px-1">
                    ...
                    <div className="flex items-center gap-3 text-[11px] text-zinc-500">
                        <span>대기 <strong className="text-zinc-300">{waitingItems.length}</strong></span>
                        <span>완료 <strong className="text-zinc-300">{doneItems.length}</strong></span>
                    </div>
                </div>
                ...
                {activeItem && (
                    <section className="space-y-1.5">
                        <p className="text-[10px] font-semibold text-amber-500/80 uppercase tracking-widest px-1">
                            ● 진행중
                        </p>
                        <ItemCard
                            item={activeItem}
