      ...
      <div className="bid-feed-scroll flex flex-col gap-1 overflow-y-auto">
        {bids.map((bid, idx) => (
          ...
          >
            ...
            <span
              className={`w-20 shrink-0 overflow-hidden text-[11px] font-bold whitespace-nowrap text-ellipsis py-0.5 ${bid.isTop ? 'text-white' : 'text-[#a1a1aa]'}`}
            >
              {bid.user}
            </span>
            <span
              className={`ml-auto shrink-0 font-mono text-[11px] font-black ${bid.isTop ? 'text-[#C5A059]' : 'text-[#71717a]'}`}
            >
              {bid.amount}
            </span>
            {bid.isTop && (
