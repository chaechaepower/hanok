...
export default function UniqueBidPanel({ bid }: Props) {
  return (
    <div className="flex min-h-[74px] flex-1 items-stretch gap-2">
      <div className="flex w-[65%] self-stretch flex-col gap-1.5">
        <div className="flex min-h-[28px] items-center justify-center rounded-lg bg-neutral-900 px-3 py-1">
          ...
          {bid.isInsufficientBalance && (
            <span className="ml-2 text-caption font-bold text-accent-light">잔고 부족</span>
          )}
        </div>

        <div
          className={`flex min-h-[40px] flex-1 items-center rounded-lg border px-3 py-1.5 transition-colors ${
            bid.isUniqueBidCorrectionPending ? 'border-accent' : 'border-transparent'
          }`}
        >
          <input
            type="text"
      ...
      <button
        type="button"
        className="flex flex-1 self-stretch items-center rounded-xl bg-gold px-3 text-neutral-100 transition hover:bg-gold-dark disabled:cursor-not-allowed disabled:opacity-45"
