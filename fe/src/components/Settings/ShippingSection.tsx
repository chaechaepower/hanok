  ...
  return (
    <>
      ...
      {modalOpen && (
        ...
        >
          <div
            className="bg-background border border-white/5 rounded-2xl w-[500px] p-8 flex flex-col gap-5 shadow-[0_8px_30px_rgba(0,0,0,0.5)] max-h-[90vh] overflow-y-auto"
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[14px] text-neutral-400 font-medium">받는 분</label>
              <input
                type="text"
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[14px] text-neutral-400 font-medium">배송지 별칭</label>
              <input
                type="text"
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[14px] text-neutral-400 font-medium">우편번호</label>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[14px] text-neutral-400 font-medium">주소</label>
              <input
                type="text"
                value={form.address}
                readOnly
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[14px] text-neutral-400 font-medium">상세 주소</label>
              <input
                type="text"
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[14px] text-neutral-400 font-medium">휴대폰 번호</label>
              <input
                type="tel"
                value={form.phone}
                onChange={(e) => {
                ...
                className={`w-full box-border bg-white/[0.02] text-white border rounded-lg px-4 py-3 text-[15px] outline-none transition-colors ${phoneError ? 'border-accent' : 'border-white/5 focus:border-gold-light'}`}
              />
              {phoneError && <p className="m-0 text-[13px] text-accent-light">{phoneError}</p>}
            </div>

            {formError && <p className="m-0 text-[13px] text-accent-light">{formError}</p>}
      ...
      {addressSearchOpen && (
        ...
        >
          <div
            className="bg-background border border-white/5 rounded-2xl w-[520px] p-8 flex flex-col gap-5 shadow-[0_8px_30px_rgba(0,0,0,0.5)] max-h-[80vh] overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <input
                ...
                autoFocus
                className="flex-1 box-border bg-white/[0.02] text-white border border-white/5 rounded-lg px-4 py-3 text-[15px] outline-none focus:border-gold-light transition-colors"
              />
              <button
            ...
            {searchResults.length === 0 && !searchError && !searchLoading && (
              <div className="flex flex-col items-center gap-3 py-8 text-neutral-500">
                ...
                <div className="text-center text-[14px] leading-relaxed">
                  ...
                  <p className="m-0 text-[13px] text-neutral-500 mt-1">예: 판교역로 235, 삼성동 159</p>
                </div>
              </div>
            )}
            ...
            {searchError && <p className="m-0 text-[13px] text-accent-light text-center py-4">{searchError}</p>}

            {!searchLoading && searchResults.length > 0 && (
              <>
                <p className="m-0 text-[13px] text-neutral-600">
                  총 <span className="text-gold-light font-semibold">{totalCount.toLocaleString()}</span>건의 결과
                </p>
                <div className="flex flex-col overflow-y-auto max-h-[340px] -mx-2">
                  {searchResults.map((juso, idx) => (
                    <button
                      ...
                      className="w-full text-left bg-transparent border-none px-4 py-3.5 cursor-pointer hover:bg-neutral-800 rounded-lg transition-colors group mx-0"
                    >
                      <div className="flex items-start gap-3">
                        <span className="inline-flex items-center px-2 py-0.5 bg-gold-light/15 text-gold-light text-[12px] font-bold rounded mt-0.5 flex-shrink-0">
                          {juso.zipNo}
                        </span>
                        <div className="flex flex-col gap-1 min-w-0">
                          <span className="text-[14px] text-white group-hover:text-gold-light transition-colors break-words">
                            {juso.roadAddr}
                          </span>
                          <span className="text-[13px] text-neutral-600 break-words">{juso.jibunAddr}</span>
                        </div>
                      </div>
                    </button>
                  ))}
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-center gap-1 pt-2">
                    <button
                      ...
                      className="px-3 py-1.5 bg-transparent border border-white/5 text-neutral-400 text-[13px] rounded cursor-pointer hover:bg-neutral-800 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                    >
                      이전
                    </button>
                    <span className="text-[13px] text-neutral-600 px-3">
